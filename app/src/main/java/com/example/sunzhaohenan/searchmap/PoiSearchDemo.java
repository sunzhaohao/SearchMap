...
public class PoiSearchDemo extends FragmentActivity implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        double latitude=getIntent().getDoubleExtra(getString(R.string.locLat),0);
        double longitude=getIntent().getDoubleExtra(getString(R.string.locLon),0);
        float accuracy=getIntent().getFloatExtra(getString(R.string.locAcc),0);
        float driection=getIntent().getFloatExtra(getString(R.string.locDir),0);

        MyLocationData locData= new MyLocationData.Builder()
                .accuracy(accuracy)
                // 此处设置开发者获取到的方向信息，顺时针0-360
                .direction(driection).latitude(latitude)
        ...
        center=new LatLng(latitude,longitude); // 周边搜索，center 为中心
